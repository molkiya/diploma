from functions.supervised_experiments import run_classifiers
from functions.visualizations import plot_learning_curves
from functions.elliptic_dataset_preprocessing import split_data, build_graph, read_data
...
def run_setup(runs, warmup_policy, hot_policy, threshold , num_iterations, batch_size, train_data):
            ...
            for hot_query, hot_policy in hot_policies.items():
                ...
                for i in range(runs):
                    ...
                    unlabeled_pool, labeled_pool = run_setup(runs = runs, warmup_policy = warmup_policy, hot_policy = hot_policy,
                                                         threshold = threshold, num_iterations=total_budget, batch_size=batch_size, train_data=train_data)
